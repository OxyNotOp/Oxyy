...
async def cbgroupdel(_,CallbackQuery):  
    ...
    _playlist = await get_note_names(CallbackQuery.message.chat.id)                                    
    if not _playlist:
        return await CallbackQuery.message.reply_text("❌ Group has no Playlist on Oxy's Server")
    else:
        titlex = []
        for note in _playlist:
...
async def delplcb(_,CallbackQuery): 
    ...
    _playlist = await get_note_names(CallbackQuery.from_user.id)                                    
    if not _playlist:
        return await CallbackQuery.message.reply_text("❌ You have no Playlist on Oxy's Server")
    else:
        titlex = []
        for note in _playlist:
